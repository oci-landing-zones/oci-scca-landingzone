...
module "default_log_service_connector" {
  ...
  tenancy_ocid          = var.tenancy_ocid
  compartment_id        = var.home_region_deployment ? module.vdms_compartment[0].compartment_id : var.backup_vdms_compartment_ocid
  source_compartment_id = var.home_region_deployment ? module.vdms_compartment[0].compartment_id : var.backup_vdms_compartment_ocid
...
module "audit_log_service_connector" {
  ...
  tenancy_ocid          = var.tenancy_ocid
  compartment_id        = var.home_region_deployment ? module.vdms_compartment[0].compartment_id : var.backup_vdms_compartment_ocid
  source_compartment_id = var.home_region_deployment ? module.home_compartment[0].compartment_id : var.backup_home_compartment_ocid
...
locals {
  svc_conn_policy_std_statements = [
    "Allow any-user to read log-content in compartment id ${var.home_region_deployment ? module.home_compartment[0].compartment_id : var.backup_home_compartment_ocid} where all {request.principal.type='serviceconnector'}",
    "Allow any-user to read log-groups in compartment id ${var.home_region_deployment ? module.home_compartment[0].compartment_id : var.backup_home_compartment_ocid} where all {request.principal.type='serviceconnector'}",
    "Allow any-user to {STREAM_READ, STREAM_CONSUME} in compartment id ${var.home_region_deployment ? module.vdms_compartment[0].compartment_id : var.backup_vdms_compartment_ocid} where all {request.principal.type='serviceconnector', target.stream.id='${module.service_event_stream.stream_id}', request.principal.compartment.id='${var.home_region_deployment ? module.vdms_compartment[0].compartment_id : var.backup_vdms_compartment_ocid}'}",
  ]
...
#  compartment_id                             = var.cloud_guard_target_tenancy ? var.tenancy_ocid : var.home_region_deployment ? module.home_compartment[0].compartment_id : var.backup_home_compartment_ocid
#  display_name                               = local.cloud_guard.display_name
#  target_resource_id                         = var.cloud_guard_target_tenancy ? var.tenancy_ocid : var.home_region_deployment ? module.home_compartment[0].compartment_id : var.backup_home_compartment_ocid
...
module "vss" { # WAS THE SPEC ASKING TO CREATE A VSS TARGET AS WELL?
  ...
  compartment_ocid                           = var.home_region_deployment ? module.vdms_compartment[0].compartment_id : var.backup_vdms_compartment_ocid
  target_compartment_ocid                    = var.home_region_deployment ? module.vdms_compartment[0].compartment_id : var.backup_vdms_compartment_ocid
...
locals {
  osms_dg_policy = {
    ...
    statements = [
      ...
      "Allow dynamic-group ${local.identity_domain.dynamic_groups.osms_dynamic_group.dynamic_group_name} to read instance-family in compartment ${module.home_compartment[0].compartment_name}",
      "Allow dynamic-group ${local.identity_domain.dynamic_groups.osms_dynamic_group.dynamic_group_name} to use osms-managed-instances in compartment ${module.home_compartment[0].compartment_name}"
    ]
  }
