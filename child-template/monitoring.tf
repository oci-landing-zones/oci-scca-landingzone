...
locals {
  ...
  topics = {
    VDMS-CRITICAL-TOPIC : {
      ...
      name           = var.deployment_type == "SINGLE" ? "VDMS-Critical-${var.resource_label}" : "CHILD-VDMS-Critical-${var.resource_label}"
      subscriptions = [
        {
          protocol = "EMAIL"
    ...
    VDMS-WARNING-TOPIC : {
      compartment_id = "SCCA_CHILD_VDMS_CMP"
      name           = var.deployment_type == "SINGLE" ? "VDMS-Warning-${var.resource_label}" : "CHILD-VDMS-Warning-${var.resource_label}"
      subscriptions = [
        {
          protocol = "EMAIL"
    ...
    VDSS-CRITICAL-TOPIC : {
      ...
      name           = var.deployment_type == "SINGLE" ? "VDSS-Critical-${var.resource_label}" : "CHILD-VDSS-Critical-${var.resource_label}"
      subscriptions = [
        {
          protocol = "EMAIL"
    ...
    VDSS-WARNING-TOPIC : {
      compartment_id = "SCCA_CHILD_VDSS_CMP"
      name           = var.deployment_type == "SINGLE" ? "VDSS-Warning-${var.resource_label}" : "CHILD-VDSS-Warning-${var.resource_label}"
      subscriptions = [
        {
          protocol = "EMAIL"
          values   = var.vdss_critical_topic_endpoints
  ...
  alarm_policy = {
    "ALARM-POLICY" : {
      name : var.deployment_type == "SINGLE" ? "OCI-SCCA-LZ-Alarm-Policy-${var.resource_label}" : "OCI-SCCA-LZ-Child-Alarm-Policy-${var.resource_label}"
      description : "OCI SCCA Landing Zone Child Alarm Policy"
      compartment_id : "SCCA_CHILD_HOME_CMP"
      statements : [
        <<EOT
        Allow group OCI-SCCA-CHILD-DOMAIN-${local.region_key[0]}-${var.resource_label}/NETWORK_ADMIN_CHILD to read metrics in compartment id ${module.scca_compartments[0].compartments["SCCA_CHILD_HOME_CMP"].id} where any {
  ...
  logging_analytics_configuration = {
    ...
    log_groups = {
      DEFAULT-LOG-ANALYTICS-LOG-GROUP = {
        ...
        compartment_id = "SCCA_CHILD_VDMS_CMP"
        name           = var.deployment_type == "SINGLE" ? "Default_Group_${var.resource_label}" : "Child_Default_Group_${var.resource_label}"
        description    = "Logging Analytics Log Group for Default_Group"
      }
      AUDIT-LOG-ANALYTICS-LOG-GROUP = {
        type           = "logging_analytics"
        compartment_id = "SCCA_CHILD_VDMS_CMP"
        name           = var.deployment_type == "SINGLE" ? "Audit_Group_${var.resource_label}" : "Child_Audit_Group_${var.resource_label}"
  ...
  enable_logging_logging_analytics_service_connector = var.enable_logging_compartment ? {
    DEFAULT-LOGGING-ANALYTICS-SERVICE-CONNECTOR = {
      ...
      policy = {
        name           = var.deployment_type == "SINGLE" ? "OCI-SCCA-LZ-Default-Logging-Analytics-Policy-${var.resource_label}"  : "OCI-SCCA-LZ-CHILD-Default-Logging-Analytics-Policy-${var.resource_label}"
        description    = "OCI SCCA Landing Zone Default Logging Analytics Policy"
        compartment-id = "SCCA_CHILD_VDMS_CMP"
      }
    }
  } : {}
  ...
  logging_analytics_service_connector = {
    AUDIT-LOGGING-ANALYTICS-SERVICE-CONNECTOR : {
      ...
      source = {
        kind = "logging"
        audit_logs = [{
          cmp_id = module.scca_compartments[0].compartments["SCCA_CHILD_VDMS_CMP"].id
        }]
      }
      target = {
        kind         = "logginganalytics"
      ...
      policy = {
        name           = var.deployment_type == "SINGLE" ?  "OCI-SCCA-LZ-Audit-Logging-Analytics-Policy-${var.resource_label}" : "OCI-SCCA-LZ-Child-Audit-Logging-Analytics-Policy-${var.resource_label}"
        description    = "OCI SCCA Landing Zone Child Audit Logging Analytics Policy"
        compartment_id = "SCCA_CHILD_HOME_CMP"
      }
    }
  }
