...
class DestroyLandingZone:
    def __init__(self, parent_cmp: str, region_key: str, resource_label: str, template_name: str, oci_config: str = "~/.oci/config", profile_name: str = "DEFAULT"):
        ...
        self.log_analytics_client = oci.log_analytics.LogAnalyticsClient(
            self.config)
        self.logging_management_client = oci.logging.LoggingManagementClient(
            self.config
        )
    ...
    def get_scca_cmps(self) -> Dict[str, str]:
        ...
        iac_cmp_name = f"OCI-SCCA-LZ-BACKUP-Config-{self.region_key}-{self.resource_label}"
    ...
    def delete_groups(self, cmp_id: str):
        list_group_response = self.identity_client.list_groups(
            compartment_id=cmp_id
        )
        groups = list_group_response.data

        for group in groups:
            delete_group_response = self.identity_client.delete_group(
                group_id=group.id
            )

    def delete_dynamic_groups(self, cmp_id: str):
        list_dynamic_group_response = self.identity_client.list_dynamic_groups(
            compartment_id=cmp_id
        )

        dynamic_groups = list_dynamic_group_response.data

        for dynamic_group in dynamic_groups:
            delete_dynamic_group_response = self.identity_client.delete_dynamic_group(
    ...
    def deactivate_domains(self, cmp_id: str):
        ...
        for domain in domains:
            ...
            wait_deactivate_domain_response = oci.wait_until(
                self.identity_client,
                self.identity_client.get_domain(domain.id),
                'lifecycle_state',
                'INACTIVE'
            )
    ...
    def delete_domains(self, cmp_id: str):
        ...
        for domain in domains:
            delete_domain_response = self.identity_client.delete_domain(
                domain_id=domain.id
        )
    ...
    def destroy_scca(self):
        ...
        print("beginning bucket destroy")
        bucket_cmps = [compartments["iac"], compartments["log"], compartments["vdms"]]
        ...
        print("deleting dynamic groups")
        self.delete_dynamic_groups(compartments["parent"])

        print("deleting groups")
        self.delete_groups(compartments["parent"])

        print("deactivating domains")
        self.deactivate_domains(compartments["vdms"])
        self.deactivate_domains(compartments["parent"])

        print("deleting domains")
        self.delete_domains(compartments["vdms"])
        self.delete_domains(compartments["parent"])
