...
variable "home_region_deployment" {
  ...
  default     = true
  validation {
    condition     = can(regex("^([t][r][u][e]|[f][a][l][s][e])$", var.home_region_deployment))
    error_message = "The home_region_deployment boolean_variable must be either true or false."
  }
}
