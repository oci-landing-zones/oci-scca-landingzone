...
locals {
  single_tenancy_home_name = "OCI-SCCA-LZ-HOME"
  single_tenancy_vdms_name = "OCI-SCCA-LZ-VDMS"
  single_tenancy_vdss_name = "OCI-SCCA-LZ-VDSS"
  single_tenancy_logging_name = "OCI-SCCA-LZ-Logging"
  ...
  home_compartment = {
    name        = var.deployment_type == "SINGLE" ? "${local.single_tenancy_home_name}-${local.region_key[0]}-${var.resource_label}" : "${var.home_compartment_name}-${local.region_key[0]}-${var.resource_label}"
    description = "Landing Zone Home Compartment"
  }

  vdms_compartment = {
    name        = var.deployment_type == "SINGLE" ? "${local.single_tenancy_vdms_name}-${local.region_key[0]}-${var.resource_label}": "${var.vdms_compartment_name}-${local.region_key[0]}-${var.resource_label}"
    description = "Virtual Datacenter Management and Security Stack"
  }

  vdss_compartment = {
    name        = var.deployment_type == "SINGLE" ? "${local.single_tenancy_vdss_name}-${local.region_key[0]}-${var.resource_label}" : "${var.vdss_compartment_name}-${local.region_key[0]}-${var.resource_label}"
  ...
  logging_compartment = {
    name        = var.deployment_type == "SINGLE" ? "${local.single_tenancy_logging_name}-${local.region_key[0]}-${var.resource_label}": "${var.logging_compartment_name}-${local.region_key[0]}-${var.resource_label}"
