...
# ###################################################################################################### #
locals {
  nfw_forwarding_ip_ocid = [try(module.scca_networking_firewall[0].provisioned_networking_resources.oci_network_firewall_network_firewalls["PARENT-NFW-KEY"].id, ""), ""][var.enable_network_firewall ? 0 : 1]

  logging_configuration_nfw = {
    default_compartment_id = var.enable_logging_compartment && var.home_region_deployment ? module.scca_compartments[0].compartments["SCCA_PARENT_LOGGING_CMP"].id : var.multi_region_logging_compartment_ocid

    log_groups = {
      DEFAULT_LOG_GROUP = {
        name           = "${var.default_log_group_name}-${var.resource_label}"
        compartment_id = var.enable_logging_compartment && var.home_region_deployment ? module.scca_compartments[0].compartments["SCCA_PARENT_LOGGING_CMP"].id : var.multi_region_logging_compartment_ocid
        description    = "${var.default_log_group_desc}-${var.resource_label}"
      }
    }
    service_logs = {
      LB_ACCESS_LOG = {
        name         = "${var.lb_access_log_name}-${var.resource_label}"
        log_group_id = "DEFAULT_LOG_GROUP"
        log_type     = "${var.lb_access_log_type}"
        category     = "${var.lb_access_log_category}"
        resource_id  = module.scca_networking_lb.provisioned_networking_resources.l7_load_balancers.l7_load_balancers["PARENT-VDMS-LB"].id
        service      = "${var.lb_access_log_service}"
      }
      LB_ERROR_LOG = {
        name         = "${var.lb_error_log_name}-${var.resource_label}"
        log_group_id = "DEFAULT_LOG_GROUP"
      ...
      OS_WRITE_LOG = {
        name         = "${var.os_write_log_name}-${var.resource_label}"
        log_group_id = "DEFAULT_LOG_GROUP"
      ...
      OS_READ_LOG = {
        name         = "${var.os_read_log_name}-${var.resource_label}"
        log_group_id = "DEFAULT_LOG_GROUP"
      ...
      VSS_LOG = {
        name         = "${var.vss_log_name}-${var.resource_label}"
        log_group_id = "DEFAULT_LOG_GROUP"
        log_type     = "${var.vss_log_type}"
        category     = "${var.vss_log_category}"
        resource_id  = module.scca_networking.provisioned_networking_resources.subnets["PARENT-VDMS-SUBNET"].id
        service      = "${var.vss_log_service}"
      }
      EVENT_LOG = {
        name         = "${var.event_log_name}-${var.resource_label}"
        log_group_id = "DEFAULT_LOG_GROUP"
      ...
      FIREWALL_THREAT_LOG = {
        name         = "${var.firewall_threat_log_name}-${var.resource_label}"
        log_group_id = "DEFAULT_LOG_GROUP"
      ...
      FIREWALL_TRAFFIC_LOG = {
        name         = "${var.firewall_traffic_log_name}-${var.resource_label}"
        log_group_id = "DEFAULT_LOG_GROUP"
  ...
  vcn_flow_logging_configuration = {
    ...
    log_groups = {
      VCN-FLOW-LOG-GROUP = {
        name = "vcn-flow-parent-log-group"
      }
    }
    ...
    flow_logs = {
      VCN-FLOW-LOGS = {
        ...
        target_compartment_ids  = ["SCCA_PARENT_VDMS_CMP"]
  ...
  svc_conn_policy_std_statements = [
    "Allow any-user to read log-content in compartment id ${var.home_region_deployment ? module.scca_compartments[0].compartments["SCCA_PARENT_HOME_CMP"].id : var.multi_region_home_compartment_ocid} where all {request.principal.type='serviceconnector'}",
    "Allow any-user to read log-groups in compartment id ${var.home_region_deployment ? module.scca_compartments[0].compartments["SCCA_PARENT_HOME_CMP"].id : var.multi_region_home_compartment_ocid} where all {request.principal.type='serviceconnector'}",
    "Allow any-user to {STREAM_READ, STREAM_CONSUME} in compartment id ${var.home_region_deployment ? module.scca_compartments[0].compartments["SCCA_PARENT_VDMS_CMP"].id : var.multi_region_vdms_compartment_ocid} where all {request.principal.type='serviceconnector', target.stream.id='${module.service_streams.streams["SERVICE-EVENT-STREAM"].id}', request.principal.compartment.id='${var.home_region_deployment ? module.scca_compartments[0].compartments["SCCA_PARENT_VDMS_CMP"].id : var.multi_region_vdms_compartment_ocid}'}",
  ]

  svc_conn_policy_log_comp_statements = var.enable_logging_compartment ? [
    "Allow any-user to manage objects in compartment id ${var.home_region_deployment ? module.scca_compartments[0].compartments["SCCA_PARENT_LOGGING_CMP"].id : var.multi_region_logging_compartment_ocid} where all {request.principal.type='serviceconnector', target.bucket.name='*_archive', request.principal.compartment.id='${var.home_region_deployment ? module.scca_compartments[0].compartments["SCCA_PARENT_VDMS_CMP"].id : var.multi_region_vdms_compartment_ocid}'}",
  ...
  service_connectors_configuration = {
    ...
    buckets = {
      DEFAULT-LOG-BUCKET = {
        ...
        compartment_id     = "SCCA_PARENT_LOGGING_CMP"
      ...
      BACKUP-BUCKET = {
        ...
        description    = "Backup bucket"
        compartment_id = "SCCA_PARENT_BACKUP_CMP"
  ...
  service_events_configuration = {
    ...
    event_rules = {
      SERVICE-EVENT-RULE = {
        is_enabled             = true
        event_display_name     = "All events in ${var.vdms_compartment_name}-${local.region_key[0]}-${var.resource_label}"
        supplied_events        = ["{}"]
